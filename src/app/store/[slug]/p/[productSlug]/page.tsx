...
async function getProduct(shopSlug: string, productSlug: string) {
  return prisma.product.findFirst({
    where: {
      slug: productSlug,
      status: "PUBLISHED",
      shop: { slug: shopSlug, status: "ACTIVE" },
    },
    include: {
      shop: {
        select: {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: product.shortDescription ?? `Buy ${product.title} from ${product.shop.name}`,
    openGraph: {
      title: `${product.title} — ${product.shop.name}`,
      description: product.shortDescription ?? "",
      images: product.mainImageUrl ? [product.mainImageUrl] : [],
      url: absoluteUrl(`/store/${slug}/p/${productSlug}`),
    },
    alternates: { canonical: absoluteUrl(`/store/${slug}/p/${productSlug}`) },
  };
}
...
export default async function ProductPublicPage({ params }: PageProps) {
  ...
  const allImages = [
    ...(product.mainImageUrl ? [{ url: product.mainImageUrl, altText: product.title }] : []),
  ...
  const avgRating =
    reviewRatings.length > 0
      ? Math.round(
          (reviewRatings.reduce((a, b) => a + b, 0) / reviewRatings.length) * 10
        ) / 10
      : null;
      ...
      <nav className="sticky top-0 z-50 glass border-b border-slate-200">
        <div className="container flex items-center justify-between h-14">
          <div className="flex items-center gap-4 flex-1 overflow-hidden pr-4">
            ...
            {product.category && (
              <>
                <span className="text-sm text-slate-500 whitespace-nowrap">{product.category.name}</span>
                <span className="text-slate-300">/</span>
              </>
            )}
            <span className="text-sm text-slate-700 truncate font-medium">{product.title}</span>
          </div>
          ...
          <div className="space-y-5">
            ...
            {avgRating && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-slate-400">({product._count.reviews} reviews)</span>
              </div>
            )}
            ...
            <div className="space-y-3 pt-2">
              {product.orderableInternally && !product.externalInquiryOnly && product.inStock && (
                <AddToCartButton 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            {product.longDescription && (
              <div className="bg-white rounded-2xl border border-slate-100 p-6">
                <h2 className="font-bold text-slate-900 mb-4">Product Details</h2>
            ...
            {/* Reviews Section */}
            <div className="bg-white rounded-2xl border border-slate-100 p-6" id="reviews-section">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                  <h2 className="font-bold text-slate-900 text-xl">Customer Reviews</h2>
                  {avgRating && (
              ...
              {product.reviews.length > 0 ? (
                <div className="space-y-2">
                  {product.reviews.map((review) => (
                    <PublicReviewItem key={review.id} review={review} type="product" />
                  ))}
                </div>
              ) : (
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-slate-100 p-5">
              <h3 className="font-bold text-slate-900 mb-4">About the Seller</h3>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="font-semibold text-slate-900">{product.shop.name}</p>
                  {product.shop.isVerified && (
